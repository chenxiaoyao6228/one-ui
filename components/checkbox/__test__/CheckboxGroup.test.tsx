...
test('should work basically', () => {
  ...
  const { getByDisplayValue } = render(
    <CheckboxGroup options={['Apple', 'Pear', 'Orange']} onChange={onChange}></CheckboxGroup>
  )
  getByDisplayValue(/Apple/i)
    .click()
  expect(onChange).toHaveBeenCalledWith(['Apple'])


  getByDisplayValue(/Pear/i)
    .click()
  expect(onChange).toHaveBeenCalledWith(['Apple', 'Pear'])


  getByDisplayValue(/Orange/i)
    .click()
...
test('does not trigger onChange callback of Checkbox and CheckboxGroup when CheckboxGroup is disabled', () => {
  ...
  const { getByDisplayValue } = render(
    <CheckboxGroup options={options} onChange={onChangeGroup} disabled></CheckboxGroup>
  )
  getByDisplayValue('Apple')
    .click()
